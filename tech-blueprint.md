...
| **Temporal** | 了解 | 持久化工作流引擎，保证长时间 Agent 任务可靠执行，适合���产级 Agentic Pipeline |
...
| 策略 | 重要程度 | 说明 |
|------|---------|------|
| **Upsert（插入/更新）** | 必学 | 基于文档���一 ID 做 Upsert，相同 ID 覆盖旧向量，是增量同步的基础操作 |
...
**研究与实现分离**：探索阶段（对比多种方案）和实现阶段必须用独立 Session。探索阶段的宽 Context 会污��实现阶段的精确执行。
...
- **Rust**（AI 基��趋势：Qdrant / Turbopack 等高性能工具已采用；若未来转向底层推理代理或向量计算基建，值得中长期投入；应用层开发暂无需学）
